...
stock_price=na.omit(stock_data$NTPC.NS$NTPC.NS.Adjusted)
#pnj_price

#stock_price = NTPC.NS$NTPC.NS.Close # Adjusted Closing Price
...
```{r}
# Augmented Dickey-Fuller (ADF) Test for Stationarity with NTPC Data
# *******************************************************************
...
```{r}
pnj_ds = diff(log(stock_price)); plot(pnj_ds) # NTPC (First)return Difference Time-Series
```

```{r}
pnj_ds=na.omit(pnj_ds)
adf_test_pnj_ds = adf.test(pnj_ds); adf_test_pnj_ds # Inference : NTPC Difference Time-Series is Stationary

```

```{r}
# Ljung-Box Test for Autocorrelation - NTPC Data
# ***********************************************

lb_test_pnj_ds = Box.test(pnj_ds); lb_test_pnj_ds # Inference : NTPC Difference (Stationary) Time-Series is Autocorrelated as NULL is rejected and p-value<0.0151 | NULL: No Auto correlation | Alternate: Auto Correlation
```
...
acf(pnj_ds) # ACF of NTPC Difference (Stationary) Series
pacf(pnj_ds) # PACF of NTPC Difference (Stationary) Series
```
